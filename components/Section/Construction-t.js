...
export default function ConstructionSection() {
  ...
  if (cacheValue && cacheValue.data) {
    ...
    //const { data } = datas;
    //console.log(allConstructionItems.length);
    // const img = datas.image;  {

    const constructItems = [];
    //console.log(items.length)
    if (allConstructionItems && allConstructionItems.length) {
      for (let i = 0; i < allConstructionItems.length; ++i) {
        const image = allConstructionItems[i].image;
        //console.log(items[i].image);
        constructItems.push(
          <div
            className="item"
            key={allConstructionItems[i].id}
            style={{ backgroundImage: ` url(${image.publicUrlTransformed})` }}
          >
            <h3>{allConstructionItems[i].name}</h3>
          </div>
        );
      }
    }
    ...
    function inHtml() {
      return (
        <div dangerouslySetInnerHTML={{ __html: `${Construction.description}` }} />
      );
    }
            ...
            <div className="container h-full mx-auto block__custom mt-12 ">
              <div className="grid grid-cols-2 gap-4 ">
                <div className="box col-span-2 ugol_b_left_top">
                  ...
                  <h2 className="section__title">{Construction.title}</h2>
